...
local function combat()
 ...
 -- Modifiers
  ...
  -- SpiritWalk
  if player.moving and lowest.health.percent < 50 and -spell(SB.SpiritWalk) == 0 then
    return cast(SB.SpiritWalk)
  end
  if player.moving and tank.health.percent < 50 and -spell(SB.SpiritWalk) == 0 then
    return cast(SB.SpiritWalk)
  end
  ...
  if group_health_percent < 80 and -spell(SB.HealingStreamTotem) == 0 then
    return cast(SB.HealingStreamTotem)
  end

  if tank.buff(SB.EarthShield).down then
  ...
  if toggle('cooldowns', false) and -spell(SB.HealingTideTotem) == 0 and (lowest.health.percent <= 20 or tank.health.percent <= 35 or group_health_percent < 50) then
    print 'CD - Healing Tide'
    return cast(SB.HealingTideTotem)
  end

  if toggle('cooldowns', false) and -spell(SB.HealingTideTotem) > 0 and -spell(SB.Ascendance) == 0 and (lowest.health.percent <= 20 or tank.health.percent <= 35 or group_health_percent < 40) then
    print 'CD - Healing Tide'
  ...
  --- dot
  if target.enemy and -spell(SB.FlameShock) == 0 and (not target.debuff(SB.FlameShock) or target.debuff(SB.FlameShock).remains <= 6) then
...
--[[
  ...
  --dps
  if -spell(SB.LavaBurst) == 0 and lowest.health.percent > 50 and tank.health.percent > 50 and target.enemy and inRange <= 2 and target.distance < 40 then
